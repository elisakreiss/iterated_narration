...
df = read_csv(here("data","01_pilot","07_revised06","raw.csv"))

clean = df %>%
        select(chain,deadend,generation,reproduction,story_text,story_title,guilty)

clean$story_text = gsub("(.{201,}?)\\s", "\\1\n", clean$story_text)
clean$reproduction = gsub("(.{61,}?)\\s", "\\1\n", clean$reproduction)
write.csv(clean, file = here("data","01_pilot","07_revised06","flowchart_data.csv"), row.names = FALSE)
