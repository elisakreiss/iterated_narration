...
csv_path = "./../../../data/02_main/01_reproduction-inLab/flowchart_data.csv"
	...
	for chain in chains:
		...
		for row in range(0,len(data_dict["story_title"])):
			...
			if chain == data_dict["chain"][row]:
				arr.append({'chain': chain, 'reproduction': data_dict["reproduction"][row], 'generation': data_dict["generation"][row], 'deadend': data_dict["deadend"][row], 'row': row})
...
new_graph('professor_free')
new_graph('professor_jail')
new_graph('scam_free')
new_graph('scam_jail')
